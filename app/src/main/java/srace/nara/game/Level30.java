...
//типа массив для диалогового окна - начало
        ...
        for(int i=0;i<10;i++){
            ...
            imageView.setImageResource(array.thirdtype_images[i]);
        }
        for(int i=0;i<10;i++){
            TextView textView = dialog.findViewById(previewtext[i]);
            textView.setText(array.thirdtype_texts[i]);
        }

        //типа массив для диалогового окна -конец
        TextView textdescription = (TextView) dialog.findViewById(R.id.textdescription);
        textdescription.setText(R.string.last_level);
...
//Кнопка "Назад" - конец
        ...
        final Animation a = AnimationUtils.loadAnimation(Level30.this,R.anim.alpha_1);
        //подключение анимации - конец
        numLeft = random.nextInt(72);//генерация числа от 0 до 9 для левой
        img_left.setImageResource(array.thirdtype_kat_images[numLeft]);//достаем из массива картинку
        numRight = random.nextInt(72);//для правой
        //чтобы числа не повторялись
        while (numLeft == numRight){
            numRight = random.nextInt(72);//заново генерируем
        }
        //чтобы числа не повторялись
        img_right.setImageResource(array.thirdtype_kat_images[numRight]);//достаём картинку для правой картинки
        numText = random.nextInt(72);//генерация числа от 0 до 9 для текста
        while ((numText != numLeft) && (numText != numRight))
        {
            numText = random.nextInt(72);//заново генерируем
        }
        random_text.setText(array.thirdtype_kat_texts[numText]);
