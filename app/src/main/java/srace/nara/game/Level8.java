...
public class Level8 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i=0;i<10;i++){
            ...
            if ((i>=0)&& (i<5)){
            imageView.setImageResource(array.images8[i]);}
            else {
        ...
        for(int i=0;i<10;i++){
            ...
            if ((i>=0)&& (i<5)){
            textView.setText(array.texts8[i]);}
            else {
...
//Кнопка "Назад" - конец
        ...
        final Animation a = AnimationUtils.loadAnimation(Level8.this,R.anim.alpha_1);
        //подключение анимации - конец
        numLeft = random.nextInt(5);//генерация числа от 0 до 9 для левой
        img_left.setImageResource(array.images8[numLeft]);//достаем из массива картинку
        numRight = random.nextInt(5);//для правой
        //чтобы числа не повторялись
        while (numLeft == numRight){
            numRight = random.nextInt(5);//заново генерируем
        }
        //чтобы числа не повторялись
        img_right.setImageResource(array.images8[numRight]);//достаём картинку для правой картинки
        numText = random.nextInt(5);//генерация числа от 0 до 9 для текста
        while ((numText != numLeft) && (numText != numRight))
        {
            numText = random.nextInt(5);//заново генерируем
        }
        random_text.setText(array.texts8[numText]);
